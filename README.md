...
| 改快捷键 | 点【设置】→ 快捷键区点【改键】，再按你想用的键（Esc 取消，退格改成不绑定）。字母和数字要留给输入框打字，所以只能绑功能键或带 Ctrl / Alt / Shift 的组合，`Ctrl` / `Shift` / `Alt` 本身不能绑（按 `Ctrl+S` 时 `Ctrl` 那一下先到，插件会继续等后面那个键）；等按键期间按下的键只归面板：不会漏给游戏，不会把当前对话推进，也不会变成输入框里的字母（在设置页试刚绑好的键同样如此），按住 `Ctrl` 触发的那次快进会被立刻停掉。绑好的组合在翻译时能正常触发（`Ctrl+S`、`Shift+R` 这类带字母的也算）。改动会写回 `livetl_config.rpy`（原文件先备份成 `.bak`），跟着项目源码走 |
...
| 快捷键 | screen 的 `key` 语句（keysym 由引擎的 `compile_event()` 编译）；事件按"后加入的先收到"分发，所以输入框比 `key` 语句先拿到按键 —— 它把命中快捷键的按键让出去、并吞掉随之而来的 `TEXTINPUT`（否则带字母的组合要么触发不了，要么在输入框里留下字符）；改键的捕获层用 `renpy.IgnoreEvent` 吃掉按键（`event()` 返回任何值都会被当成"本次交互结束"，say 会就此结束、剧情前进一句）；按住 Ctrl 触发的那次快进会被立刻停掉（`config.skipping` 未文档化，封在引擎适配层） |
